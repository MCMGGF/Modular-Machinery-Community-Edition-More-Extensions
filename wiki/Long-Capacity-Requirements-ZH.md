...
MMCEME 通过 Mixin 改造需求系统，使流体/气体配方量以 **`long`** 解析与处理（上限约 920 京）。覆盖整个配方生命周期：
...
- `FluidStack` / `GasStack` 本身仍是 `int` 基础；MMCEME 额外维护一个并行的 `long` 值，仅在把数据交给原版 `int` API 时才向下截断。对配方作者透明，但调试时值得知道。
