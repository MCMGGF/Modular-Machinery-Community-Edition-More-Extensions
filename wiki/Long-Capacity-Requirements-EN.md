...
MMCEME patches the requirement system (via Mixins) so fluid/gas recipe amounts are parsed and processed as **`long`** (up to 9.2 quintillion). This runs across the whole recipe lifecycle:
...
- `FluidStack` / `GasStack` themselves remain `int`-based; MMCEME keeps a parallel `long` value and only down-casts when handing data to vanilla `int` APIs. This is transparent for recipe authors but worth knowing when debugging.
